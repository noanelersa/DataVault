...
void MapSet(const char* key, const MapEntryValue* value) {
    if (!key || !value || !value->fileHash) return;
    ...
    while (entry) {
        if (strcmp(entry->key, key) == 0) {
            char* newFileHash = strdup(value->fileHash);
            if (!newFileHash) {
                fprintf(stderr, "Error: Failed to duplicate fileID string.\n");
                return;
            }

            free(entry->value->fileHash);
            entry->value->fileHash = newFileHash;
            return;
        }
        entry = entry->next;
    }
    ...
    entry->value->fileHash = strdup(value->fileHash);
    if (!entry->value->fileHash) {
        fprintf(stderr, "Error: Failed to duplicate fileHash string.\n");
        free(entry->value);
        free(entry->key);
        free(entry);
        return;
