...
BOOLEAN HandleUIFileRegister(char* recvbuf, int recvbuflen, const char* username)
{
    ...
    uint8_t fileHash[HASH_SIZE + 1] = { 0 };
    if (ComputeFileSha256(protectedFilePath, fileHash) != 0)
    {
    ...
    snprintf(jsonData, sizeof(jsonData),
        "{ \"owner\": \"%s\", \"fileHash\": \"%.32s\", \"fileName\":\"%s\",\"acl\":\"%s\" }",
        username, fileHash, protectedFilePath, jsonAclString);
