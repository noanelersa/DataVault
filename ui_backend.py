...
def register():
    ...
    try:
        register_data = AgentActionType.REGISTER_FILE.value.to_bytes(1,byteorder='big') + f"{BASE_PATH}{newFile['name']}$".encode() + register_data.encode() + b"$"
...
def update_permissions():
    ...
    try:
        ...
        register_data = AgentActionType.UPDATE_PERMISSIONS.value.to_bytes(1,byteorder='big') + f"{BASE_PATH}{file_data['name']}$".encode() + register_data.encode() + b"$"
        send_to_agent(register_data)
        return {"status": "success"}
    except Exception as e:
...
def delete_file(file_id):
    try:
        delete_data = AgentActionType.DELETE_FILE.value.to_bytes(1,byteorder='big') + f"{BASE_PATH}{file_id}$".encode()
